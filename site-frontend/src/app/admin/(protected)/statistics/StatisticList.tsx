...
import { reorderStatistics, toggleStatisticActive, deleteStatistic } from "@/lib/statistics/actions";
...
export default function StatisticList({ statistics }: { statistics: StatisticListItem[] }) {
  const router = useRouter();
  // Same "adjust state during render" pattern as BannerList.tsx/ServiceList.tsx.
  const [renderedStatistics, setRenderedStatistics] = useState(statistics);
  const [items, setItems] = useState(statistics);
  if (statistics !== renderedStatistics) {
    setRenderedStatistics(statistics);
    setItems(statistics);
  }
  ...
  async function persist(nextOrder: StatisticListItem[]) {
    setSaveState("saving");
    setErrorMessage(null);
    const result = await reorderStatistics(nextOrder.map((s) => s.id));
    if (result.error) {
      setSaveState("error");
      ...
      <Reorder.Group as="ul" axis="y" values={items} onReorder={setItems} className="list-none space-y-3">
        {items.map((statistic, index) => (
          <StatisticRow
            ...
            onMoveUp={() => moveByArrow(statistic.id, "up")}
            onMoveDown={() => moveByArrow(statistic.id, "down")}
            onToggled={(active) =>
              setItems((cur) => cur.map((s) => (s.id === statistic.id ? { ...s, active } : s)))
            }
            onDeleted={() => {
              setItems((cur) => cur.filter((s) => s.id !== statistic.id));
              router.refresh();
            }}
          />
        ))}
    ...
    >
      <div className="flex min-w-0 items-center gap-3">
        <button
          ...
          aria-label={`Arrastar para reordenar "${statistic.label}"`}
        ...
        <div className="flex h-12 w-20 shrink-0 items-center justify-center rounded-lg bg-[#f7f6f6] text-sm font-bold text-primary">
          {statistic.prefix}
          {statistic.value}
          {statistic.suffix}
        </div>
        ...
        <div className="min-w-0 flex-1">
          <span className="block truncate font-semibold text-foreground">{statistic.label}</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 pl-[52px] sm:ml-auto sm:pl-0">
        <span
          className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${
            statistic.active ? "bg-primary/10 text-primary" : "bg-gray-light/60 text-gray-medium"
          }`}
        >
        ...
        <div className="flex shrink-0 items-center gap-3 border-l border-gray-light/70 pl-3">
          ...
          <Link
            href={`/admin/statistics/${statistic.id}`}
          ...
          <button
            ...
            onClick={() => {
              setActionError(null);
              setIsPending(true);
              toggleStatisticActive(statistic.id, !statistic.active)
                .then((result) => {
                  if (result.error) setActionError(result.error);
                  else onToggled(!statistic.active);
                })
                .finally(() => setIsPending(false));
            }}
            className="text-sm font-semibold text-gray-medium hover:text-foreground disabled:opacity-60"
          >
            {statistic.active ? "Desativar" : "Ativar"}
          </button>
          <button
            type="button"
      ...
      <ConfirmDialog
        ...
        title="Excluir esta estatística?"
        description={`"${statistic.label}" será removida definitivamente.`}
        confirmLabel="Excluir"
        danger
        pending={isPending}
        onConfirm={() => {
          setIsPending(true);
          deleteStatistic(statistic.id)
            .then((result) => {
              if (result.error) setActionError(result.error);
              else onDeleted();
            })
